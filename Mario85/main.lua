---@diagnostic disable: redundant-parameter, undefined-doc-name
-- name: \\#4d0000\\ MX moveset v1.6 
-- description: mod made by Dremy_Bowser  play as the scary guy himself!!!!!      the moveset is off by default!!!      turn it on with /M 85 and turn it off with /M 64                    Ft. Birdikek (for his reduced knockback)

gPlayerSyncTable[0].movesetActive = false

function moveset_active(m)
    return gPlayerSyncTable[0].movesetActive and m.character.type == CT_MARIO
end
...
function air_recovery(m)
    if not moveset_active(m) then return end
    if m.action == ACT_BACKWARD_AIR_KB or m.action == ACT_FORWARD_AIR_KB or m.action == ACT_BACKWARD_GROUND_KB or m.action == ACT_FORWARD_GROUND_KB
    then
